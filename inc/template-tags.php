 ...
 */
function territory_theme_post_date( $args = [] ) {

	// Set defaults.
	$defaults = [
		'date_text'   => esc_html__( 'Posted on', 'territory-theme' ),
 ...
 * @param array $args Configuration args.
 */
function territory_theme_post_author( $args = [] ) {

	// Set defaults.
	$defaults = [
		'author_text' => esc_html__( 'by', 'territory-theme' ),
	];
 ...
 * @author WebDevStudios
 */
function territory_theme_entry_footer() {
	// Hide category and tag text for pages.
	if ( 'post' === get_post_type() ) {
		/* translators: used between list items, there is a space after the comma */
		$categories_list = get_the_category_list( esc_attr__( ', ', 'territory-theme' ) );
		if ( $categories_list && territory_theme_categorized_blog() ) {

			/* translators: the post category */
			printf( '<span class="cat-links">' . esc_attr__( 'Posted in %1$s', 'territory-theme' ) . '</span>', $categories_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_attr__( ', ', 'territory-theme' ) );
		if ( $tags_list ) {

			/* translators: the post tags */
			printf( '<span class="tags-links">' . esc_attr__( 'Tagged %1$s', 'territory-theme' ) . '</span>', $tags_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- XSS OK.
		}
	}

	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		echo '<span class="comments-link">';
		comments_popup_link( esc_attr__( 'Leave a comment', 'territory-theme' ), esc_attr__( '1 Comment', 'territory-theme' ), esc_attr__( '% Comments', 'territory-theme' ) );
		echo '</span>';
	}
	...
	edit_post_link(
		sprintf(
			...
			esc_html__( 'Edit %s', 'territory-theme' ),
 ...
 */
function territory_theme_display_svg( $args = [] ) {
 ...
 */
function territory_theme_get_svg( $args = [] ) {
	// Make sure $args are an array.
	if ( empty( $args ) ) {
		return esc_attr__( 'Please define default parameters in the form of an array.', 'territory-theme' );
	}

	// Define an icon.
	if ( false === array_key_exists( 'icon', $args ) ) {
		return esc_attr__( 'Please define an SVG icon filename.', 'territory-theme' );
	}
	...
	<svg
	<?php
		echo territory_theme_get_the_content( $height ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- XSS OK.
		echo territory_theme_get_the_content( $width ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- XSS OK.
		echo territory_theme_get_the_content( $color ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- XSS OK.
		echo territory_theme_get_the_content( $stroke_width ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- XSS OK.
	?>
		class="icon <?php echo esc_attr( $args['icon'] ); ?>"
	<?php
		echo territory_theme_get_the_content( $aria_hidden ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- XSS OK.
		echo territory_theme_get_the_content( $aria_labelledby ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- XSS OK.
	?>
		role="img">
 ...
 */
function territory_theme_get_the_title( $args = [] ) {
	// Set defaults.
	$defaults = [
		'length' => 12,
		'more'   => '...',
 ...
 */
function territory_theme_get_the_excerpt( $args = [] ) {
 ...
 * @author WebDevStudios
 */
function territory_theme_display_copyright_text() {
	// Grab our customizer settings.
	$copyright_text = get_theme_mod( 'territory_theme_copyright_text' );

	if ( $copyright_text ) {
		echo territory_theme_get_the_content( do_shortcode( $copyright_text ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- XSS OK.
	}
}
 ...
 * @author WebDevStudios
 */
function territory_theme_display_social_network_links() {
	...
	<ul class="flex social-icons menu">
		...
		foreach ( $social_networks as $network ) :
			...
			$network_url = get_theme_mod( 'territory_theme_' . $network . '_link' );
			...
			if ( ! empty( $network_url ) ) :
				...
				<li class="social-icon <?php echo esc_attr( $network ); ?> mr-2">
					<a href="<?php echo esc_url( $network_url ); ?>">
						...
						printf( esc_attr__( 'Link to %s', 'territory-theme' ), ucwords( esc_html( $network ) ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- XSS OK.
						?>
						</span>
					</a>
				</li><!-- .social-icon -->
 ...
 */
function territory_theme_display_numeric_pagination( $args = [], $query = null ) {
	if ( ! $query ) {
		global $wp_query;
		$query = $wp_query;
	}
	...
	<nav class="container pagination-container" aria-label="<?php esc_attr_e( 'numeric pagination', 'territory-theme' ); ?>">
 ...
 */
function territory_theme_display_mobile_menu() {
	...
	<nav class="off-canvas-container" aria-label="<?php esc_attr_e( 'Mobile Menu', 'territory-theme' ); ?>" aria-hidden="true" tabindex="-1">
		<?php
		// Mobile menu args.
		$mobile_args = [
 ...
 * @author WebDevStudios
 */
function territory_theme_display_comments() {
