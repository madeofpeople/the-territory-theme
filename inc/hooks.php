 ...
 */
function territory_theme_body_classes( $classes ) {
...
add_filter( 'body_class', 'territory_theme_body_classes' );

/**
 * Flush out the transients used in territory_theme_categorized_blog.
 *
 * @author WebDevStudios
 *
 * @return bool Whether or not transients were deleted.
 */
function territory_theme_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return false;
	}

	// Like, beat it. Dig?
	return delete_transient( 'territory_theme_categories' );
}

add_action( 'delete_category', 'territory_theme_category_transient_flusher' );
add_action( 'save_post', 'territory_theme_category_transient_flusher' );
 ...
 */
function territory_theme_content_more_link() {
	return ' <a class="more-link" href="' . get_permalink() . '">' . esc_html__( 'Read More', 'territory-theme' ) . '...</a>';
}

add_filter( 'the_content_more_link', 'territory_theme_content_more_link' );
 ...
 * @return string Read more link.
 */
function territory_theme_excerpt_more( $more ) {
	return sprintf( ' <a class="more-link" href="%1$s">%2$s</a>', get_permalink( get_the_ID() ), esc_html__( 'Read more...', 'territory-theme' ) );
}

add_filter( 'excerpt_more', 'territory_theme_excerpt_more' );
 ...
 */
function territory_theme_get_the_content( $content ) {
	return ! empty( $content ) ? $content : false;
}

add_filter( 'the_content', 'territory_theme_get_the_content', 20 );
 ...
 */
function territory_theme_custom_mime_types( $mimes ) {
...
add_filter( 'upload_mimes', 'territory_theme_custom_mime_types' );
 ...
 * @author WebDevStudios
 */
function territory_theme_include_svg_icons() {
...
add_action( 'wp_footer', 'territory_theme_include_svg_icons', 9999 );
 ...
 */
function territory_theme_display_customizer_header_scripts() {
	// Check for header scripts.
	$scripts = get_theme_mod( 'territory_theme_header_scripts' );
	...
	echo territory_theme_get_the_content( $scripts );
}

add_action( 'wp_head', 'territory_theme_display_customizer_header_scripts', 999 );
 ...
 */
function territory_theme_display_customizer_footer_scripts() {
	// Check for footer scripts.
	$scripts = get_theme_mod( 'territory_theme_footer_scripts' );
...
add_action( 'wp_footer', 'territory_theme_display_customizer_footer_scripts', 999 );
 ...
 */
function territory_theme_add_og_tags() {
	...
	$default_base_description = ( get_bloginfo( 'description' ) ) ? get_bloginfo( 'description' ) : esc_html__( 'Visit our website to learn more.', 'territory-theme' );
	...
	if ( is_category() || is_tag() || is_tax() ) {
		...
		$specify        = ( is_category() ) ? esc_html__( 'categorized in', 'territory-theme' ) : esc_html__( 'tagged with', 'territory-theme' );
		$queried_object = get_queried_object();
		$card_url       = get_term_link( $queried_object );
		$card_type      = 'website';

		// Translators: get the term name.
		$card_long_description = sprintf( esc_html__( 'Posts %1$s %2$s.', 'territory-theme' ), $specify, $term_name );
	...
	if ( is_search() ) {
		...
		$card_long_description = sprintf( esc_html__( 'Search results for %s.', 'territory-theme' ), $search_term );
		$card_description      = $card_long_description;
	}
...
add_action( 'wp_head', 'territory_theme_add_og_tags' );
 ...
 */
function territory_theme_remove_archive_title_prefix( $block_title ) {
...
add_filter( 'get_the_archive_title', 'territory_theme_remove_archive_title_prefix' );
 ...
 * @author Corey Collins
 */
function territory_theme_disable_wpautop_for_gutenberg() {
...
add_filter( 'init', 'territory_theme_disable_wpautop_for_gutenberg', 9 );
